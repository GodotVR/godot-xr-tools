...
## How far to the left the joystick can move
@export var limit_x_minimum := -45.0: set = set_limit_x_minimum

## How far to the right the joystick can move
@export var limit_x_maximum := 45.0: set = set_limit_x_maximum

## How far downwards the joystick can move
@export var limit_y_minimum := -45.0: set = set_limit_y_minimum

## How far upwards the joystick can move
@export var limit_y_maximum := 45.0: set = set_limit_y_maximum
...
func _get_configuration_warnings() -> PackedStringArray:
	...
	if get_children().all(
			func(n: Node) -> bool:
				return not n is XRToolsInteractableJoystick
	):
...
func is_xr_class(xr_name: String) -> bool:
	return xr_name == "XRToolsInteractableJoystickOrigin"


## Handles setting the maximum X limit
...
## Handles setting the minimum X limit
...
## Handles setting the maximum Y limit
...
## Handles setting the minimum Y limit
