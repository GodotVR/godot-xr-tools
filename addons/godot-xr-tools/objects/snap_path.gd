...
## or at discrete intervals by setting [member snap_interval] above '0.0'.
##
...
## Enabled when not 0
@export_custom(PROPERTY_HINT_NONE, "suffix:m") var snap_interval := 0.0:
	set(v):
...
func has_snap_interval() -> bool:
	return not is_equal_approx(snap_interval, 0.0)


# Returns -1 if invalid
# _offset should be in _curve's local coordinates
func _find_closest_offset_with_length(
		_curve: Curve3D,
		_offset: float,
		_length: float,
) -> float:
...
func _get_snap_guide(target: Node3D) -> Node3D:
	for c: Node in target.get_children():
...
func _make_temp_zone() -> XRToolsSnapZone:
	...
	if has_node("AudioStreamPlayer3D"):
		zone.has_picked_up.connect(
			func(object):
				_audio_player.stream = stash_sound
				_audio_player.play()
		)

	# Copy over the properties to the XRToolsSnapZone
	zone.enabled        = true
	...
	# Copy over the properties to the CollisionObject3D
...
func _on_target_dropped(target: Node3D) -> void:
	...
	if (
			not enabled
			or not path
			or not target.can_pick_up(self)
	...
	var zone: XRToolsSnapZone = _make_temp_zone()
	var offset: float = _find_offset(path, target.global_position)
...
func _snappedf_up(x: float, step: float) -> float:
