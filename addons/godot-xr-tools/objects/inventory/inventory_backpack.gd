@icon("res://addons/godot-xr-tools/editor/icons/backpack.svg")
@tool
class_name XRToolsInventoryBackpack
extends Node
...
func is_xr_class(name : String) -> bool:
	return name == "XRToolsInventoryBackpack"
...
func _ready():
	...
	backpack.picked_up.connect(_on_backpack_picked_up)
	backpack.dropped.connect(_on_backpack_dropped)

	# Get all the snap-zones
	_backpack_snap_zones = XRTools.find_xr_children(backpack, "*", "XRToolsSnapZone")
