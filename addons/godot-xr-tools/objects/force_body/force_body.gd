...
##
## This script enhances [AnimatableBody3D] with [method move_and_slide] and the ability
## to push bodies by emparting forces on them.


## Toggles pushing bodies
@export var push_bodies := true

## Stiffness of the body
@export var stiffness := 10.0

## Maximum push force
@export var maximum_force := 1.0
...
## Moves and slides along the [param move] vector. It returns information about
## the last collision, or null if no collision
...
func rotate_and_collide(
	...
	var from_quat := Quaternion(global_basis)
	var to_quat := Quaternion(target_global_basis)
	var angle := from_quat.angle_to(to_quat)
	var steps := ceil(angle / step_angle)

	# Convert collision exceptions to a RID array
	var exception_rids: Array[RID]
	for collision_exception: PhysicsBody3D in get_collision_exceptions():
		# It is our responsibility to remove exceptions before freeing the
		# object, but sometimes that is hard.
	...
	var step := 0.0
	var new_quat := from_quat
	var t := global_transform
	while step < steps and not ret:
		step += 1.0

		var test_quat := from_quat.slerp(to_quat, step / steps)
		t.basis = Basis(test_quat)
		query.transform = t

		for rid: RID in shape_rids:
			query.shape_rid = rid
			var collision: Dictionary = direct_state.get_rest_info(query)
