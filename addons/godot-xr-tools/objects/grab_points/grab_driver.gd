...
## How objects should be grabbed
enum GrabState {
	LERP, ## Objects should lerp from their original position to the grab point
	SNAP, ## Objects should instantly snap to the grab-point
}


## Drive state
var state := GrabState.SNAP
...
## Lerp total duration
var lerp_duration := 1.0

## Lerp time
var lerp_time := 0.0


## Creates the driver to lerp the target from its current location to the
## primary grab-point.
...
## Creates the driver to instantly snap to the primary grab-point.
...
# Calculates the lerp voting from a to b
...
## Gets the grab information for the grab node
...
# Updates the weight on collision hands
func _update_weight() -> void:
	if primary:
		var weight := target.mass
		if secondary:
