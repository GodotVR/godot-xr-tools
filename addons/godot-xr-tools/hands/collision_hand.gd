...
## Emitted when our teleport distance is reached
...
## Displacement required for the hand to start orienting to a surface
...
## How the hand should collide with [PhysicsBody3D]s
@export var mode := CollisionHandMode.COLLIDE
...
@export_range(1.0, 1000.0, 0.1, "suffix:N") var min_pickup_force := 15.0

## Force we exert on a picked up object when hand is at maximum distance
## before letting go.
@export_range(1.0, 1000.0, 0.1, "suffix:N") var max_pickup_force := 300.0


# Controller to target (if no target overrides)
var _controller: XRController3D

# Sorted stack of [TargetOverride]s
...
# Weight held by this hand
var _held_weight := 0.0

# Movement on last frame
var _last_movement := Vector3()


## Searches an [XRToolsCollisionHand] that is an ancestor of the given node.
static func find_ancestor(node: Node) -> XRToolsCollisionHand:
	return XRTools.find_xr_ancestor(
			node,
			"*",
			"XRToolsCollisionHand",
	) as XRToolsCollisionHand


## Searches from the specified node for an [XRToolsCollisionHand],
...
## Searches from the specified node for the left controller
...
## Searches from the specified node for the right controller
## [XRToolsCollisionHand], assuming the node is a sibling of the [XROrigin3D].
...
# Returns warnings related to this node
...
## Updates the weight attributed to this hand (updated from pickable system).
func set_held_weight(new_weight: float) -> void:
...
# Moves the collision hand to the target node.
func _move_to_target(delta: float) -> void:
	...
	if _held_weight > 0.0:
		...
		var gravity: Vector3 = gravity_state.total_gravity * delta
		...
		var remaining_movement: Vector3 = target_movement - base_movement
		...
		var force := min_pickup_force + (
				target_movement.length()
				* (max_pickup_force - min_pickup_force)
				/ TELEPORT_DISTANCE
		)
...
# If our player moved, attempts to move our hand but ignoring weight.
...
# Moves if the player just teleported
...
# Updates our finger digits when our skeleton updates
func _on_skeleton_updated() -> void:
	if not hand_skeleton:
		return

	var bone_count: int = hand_skeleton.get_bone_count()
	for i: int in bone_count:
		var collision_node: CollisionShape3D
		var offset: Transform3D
		offset.origin = Vector3(0.0, 0.015, 0.0) # move to side of joint

		var bone_name: String = hand_skeleton.get_bone_name(i)
...
# Updates hand skeleton, if set
func _update_hand_skeleton() -> void:
	if hand_skeleton:
		if hand_skeleton.has_signal("skeleton_updated"):
			# Godot 4.3+
