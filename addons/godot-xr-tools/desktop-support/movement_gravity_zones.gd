...
func physics_pre_movement(_delta: float, player_body: XRToolsPlayerBody) -> void:
	...
	var gravity_zones1: Array[Node] = get_tree().get_nodes_in_group("gravity_zone1")
	var gravity_zones2: Array[Node] = get_tree().get_nodes_in_group("gravity_zone2")
	var gravity_zones3: Array[Node] = get_tree().get_nodes_in_group("gravity_zone3")
	...
	if grav_z == 0:
		# If there are no Area3Ds in the group gravity_zone3
	...
	if grav_z == 0:
		# If there are no Area3Ds in the group gravity_zone2
	...
	if grav_z == 0:
		# If there are no Area3Ds in the gravity_zone groups
		fly_desktop.set_flying(true)
		fly_xr.set_flying(true)
	else:
		# If there are Area3Ds in the gravity_zone groups
