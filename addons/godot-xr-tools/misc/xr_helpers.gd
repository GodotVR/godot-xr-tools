...
## somewhere under the player's [XROrigin3D].


## Finds the Left Hand's [XRController3D] from a player node and an optional path
static func get_left_controller(
		node: Node,
...
## Finds the Right Hand's [XRController3D] from a player node and an optional path
static func get_right_controller(
		node: Node,
		path: NodePath = NodePath(),
) -> XRController3D:
...
## Finds the [XRCamera3D] node by searching from the provided node, assuming the
## node is under the [XROrigin3D]. The caller may provide an optional path
## (relative to the node) to the [XRCamera3D] to support out-of-tree searches.
static func get_xr_camera(
		node: Node,
		path: NodePath = NodePath(),
) -> XRCamera3D:
...
## Finds the [XRController3D] node by searching from the provided node, assuming
## the node is under the [XRController3D]. The caller may provide an optional
## path (relative to the node) to the [XRController3D] to support out-of-tree
## searches.
...
## Finds the [XROrigin3D] node by searching from the provided node, assuming
## the node is under the [XROrigin3D]. The caller may provide an optional path
## (relative to the node) to the [XROrigin3D] to support out-of-tree searches.
static func get_xr_origin(
		node: Node,
		path: NodePath = NodePath(),
) -> XROrigin3D:
...
# Finds an [XRController3D], given some search parameters
